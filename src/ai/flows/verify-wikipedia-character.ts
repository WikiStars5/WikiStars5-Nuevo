...
const WikipediaVerificationOutputSchema = z.object({
  ...
  title: z.string().nullable().describe('The exact title of the Wikipedia page found.'),
...
export async function verifyWikipediaCharacter(
  ...
  try {
    ...
    if (!searchResult.query.search.length) {
      return { found: false, title: null, imageUrl: null, verificationError: 'No se encontró ninguna página en Wikipedia con ese nombre.', source: 'Wikipedia' };
    }
    ...
    if (similarityScore < 0.7) {
        return { found: false, title: null, imageUrl: null, verificationError: `El nombre encontrado ("${pageTitle}") no es suficientemente similar al que buscaste.`, source: 'Wikipedia' };
    }
    ...
    if (!wikitext) {
      return { found: false, title: pageTitle, imageUrl: null, verificationError: 'No se pudo obtener el contenido de la página de Wikipedia.', source: 'Wikipedia' };
    }
    ...
    if (!infoboxMatch) {
        return { found: false, title: pageTitle, imageUrl: null, verificationError: 'El artículo no contiene un cuadro de información (Ficha/Infobox), por lo que no puede ser validado.', source: 'Wikipedia' };
    }
    ...
    if (!hasKeywords) {
        return { found: false, title: pageTitle, imageUrl: null, verificationError: `El cuadro de información no contiene datos de un personaje (ej: "${CHARACTER_KEYWORDS[0]}").`, source: 'Wikipedia' };
    }
