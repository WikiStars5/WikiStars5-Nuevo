...
function BiasMemberCard({ 
    ...
    member: (BtsBiasMember & { voteCount: number });
...
export default function BtsBiasVoting() {
    ...
    const membersQuery = useMemoFirebase(() => {
        ...
        return query(collection(firestore, 'bts_bias_members'), orderBy('order'));
    }, [firestore]);

    const { data: biasMembers, isLoading: isLoadingMembers, refetch: refetchMembers } = useCollection<BtsBiasMember>(membersQuery);

    const userVoteDocRef = useMemoFirebase(() => {
        if (!firestore || !user) return null;
        return doc(firestore, `users/${user.uid}/btsBiasVote`, 'bts-bias-battle');
    }, [firestore, user]);

    const { data: userVote, refetch: refetchUserVote } = useDoc<BtsBiasVote>(userVoteDocRef, { enabled: !!user });
    ...
    useEffect(() => {
        ...
        const fetchVoteCounts = async () => {
            ...
            const combined = biasMembers
                .map(member => ({
                    ...member,
                    voteCount: figuresData.get(member.figureId)?.btsBiasVoteCount || 0,
                }))
    ...
    const handleAdd = async (figure: Figure) => {
        ...
        try {
            await addDoc(collection(firestore, 'bts_bias_members'), newMemberData);
    ...
    const handleRemove = async (docId: string) => {
        if (!firestore) return;
        try {
            await deleteDoc(doc(firestore, 'bts_bias_members', docId));
    ...
    const handleVote = async (selectedFigureId: string) => {
        ...
        try {
            await runTransaction(firestore, async (transaction) => {
                const privateVoteRef = doc(firestore, `users/${currentUser!.uid}/btsBiasVote`, 'bts-bias-battle');
                ...
                if (existingVoteFigureId) {
                    ...
                    transaction.update(oldFigureRef, { btsBiasVoteCount: increment(-1) });
                }
