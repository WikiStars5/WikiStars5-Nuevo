...
// **IMPORTACIONES CRÍTICAS** para llamar a la función
import { getFunctions, httpsCallable } from 'firebase/functions';
import { onAuthStateChanged, User as FirebaseUser, Auth, getAuth } from 'firebase/auth'; // Añadido getAuth
import { Button } from '@/components/ui/button';
import { Skeleton } from '@/components/ui/skeleton';
import { Loader2 } from 'lucide-react'; // Eliminadas imports no usadas
...
  const [isVoting, setIsVoting] = useState<AttitudeOption | null>(null);
    ...
    // CRÍTICO: Inicialización de la Cloud Function Callable
    const voteCallable = useMemoFirebase(() => {
        if (!auth) return null; 
        const app = auth.app; 
        // ¡Usamos la región donde se desplegó la función! (southamerica-east1)
        const functionsInstance = getFunctions(app, 'southamerica-east1'); 
        return httpsCallable(functionsInstance, 'handleVote');
    }, [auth]);
...
// ------------------------------------------------------------------
// FUNCIÓN handleVote: Llama al Servidor
// ------------------------------------------------------------------
  const handleVote = async (vote: AttitudeOption) => {
    // Aseguramos que la referencia a la función exista
    if (isVoting || !auth || !voteCallable) return; 
...
        
        // 1. Asegura que el usuario esté logueado (anónimo o no)
        if (!currentUser) {
            await initiateAnonymousSignIn(auth);
            currentUser = await getNextUser(auth);
        }
        
        // 2. Llama a la Cloud Function con los datos del voto
        const result = await voteCallable({
            figureId: figure.id,
            voteValue: vote
        });
        
        // 3. Maneja la respuesta del servidor (Cloud Function)
...
        
    } catch (error: any) {
        console.error('Error al registrar el voto (Cloud Function):', error);
        toast({
            variant: 'destructive',
