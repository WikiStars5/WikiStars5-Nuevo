...
export async function verifyFamousBirthdaysCharacter(
  ...
  try {
    if (!/^(https?:\/\/)?(www\.|es\.)?famousbirthdays\.com\/people\//.test(input.url)) {
        return {
            found: false,
            title: null,
            imageUrl: null,
    ...
    if (!normalizedHtml.includes(normalizedName)) {
      return {
        ...
        title: null,
        imageUrl: null,
        verificationError: `El nombre "${input.name}" no se encontró en el contenido de la página.`,
        source: 'Famous Birthdays',
      };
    }
    ...
    if (!characterName) {
        ...
        characterName = h1Match ? h1Match[1].trim() : input.name; // Fallback to input name
    }
    ...
    console.error('Error in verifyFamousBirthdaysCharacter:', error);
    return {
      found: false,
      title: null,
      imageUrl: null,
